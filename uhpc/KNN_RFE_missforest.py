...
class KNN_RFE_MissForest:
    ...
    @staticmethod
    def filled_df_except_miss_col(df_with_null: pd.DataFrame, missing_col: str, except_missing = False) -> pd.DataFrame:
        """
    ...
    def fit(self, x: pd.DataFrame):
        ...
        for _ in loop:
            ...
            for c in missing_indices:
                ...
                x_imp = self.filled_df_except_miss_col(x_end, c)
            ...
            if any(self._categorical):
                x_imp_cat.append(x_end[self._categorical])
            ...
            if any(self._numerical):
                x_imp_num.append(x_end[self._numerical])
        ...
        for i in loop:
            ...
            for feature, estimator in self._estimators[i].items():
                if x[feature].isnull().any():
                    ...
                    x_imp = self.filled_df_except_miss_col(x_end, feature)
                    if self.feature_selection:
                    ...
                    x_end.loc[missing_indices[feature], feature] = (
                        estimator.predict(x_obs).tolist()
                    )
            x_end = x_end[self.original_column_order]
            # Store imputed categorical and numerical features after
            # each iteration.
            if any(self._categorical):
                x_imp_cat.append(x_end[self._categorical])
