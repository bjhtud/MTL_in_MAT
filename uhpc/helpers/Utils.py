...
def MNAR_mask_logistic(
    ...
    idxs_params = (
        rng.choice(d, d_params, replace=False) if exclude_inputs else np.arange(d)
    )
    idxs_nas = (
    ...
    # Pick coefficients so that W^Tx has unit variance (avoids shrinking)
    coeffs = pick_coeffs(X, idxs_params, idxs_nas, random_state=random_state)
    # Pick the intercepts to have a desired amount of missing values
    ...
    if exclude_inputs:
        mask[:, idxs_params] = rng.random((n, d_params)) < p

    return mask


def MNAR_self_mask_logistic(
    X: np.ndarray, p: float, random_state: Optional[int] = None
) -> np.ndarray:
    """
    ...
    # Pick coefficients so that W^Tx has unit variance (avoids shrinking)
    coeffs = pick_coeffs(X, self_mask=True, random_state=random_state)
    # Pick the intercepts to have a desired amount of missing values
...
def simulate_nan(
    ...
    if mecha == "MAR":
        mask = MAR_mask(
            X, p_miss, p_obs, sample_columns=sample_columns, random_state=random_state
        ).astype(float)
    elif mecha == "MNAR" and opt == "logistic":
        mask = MNAR_mask_logistic(X, p_miss, p_obs, random_state=random_state).astype(
            float
        )
    elif mecha == "MNAR" and opt == "quantile":
        mask = MNAR_mask_quantiles(
            X, p_miss, q, 1 - p_obs, random_state=random_state
        ).astype(float)
    elif mecha == "MNAR" and opt == "selfmasked":
        mask = MNAR_self_mask_logistic(X, p_miss, random_state=random_state).astype(
            float
        )
    else:
        mask = (rng.random(X.shape) < p_miss).astype(float)
